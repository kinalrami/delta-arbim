...
const headingClass = "font-mono text-xs font-bold uppercase tracking-widest text-orange-400";
...
export function ContactStrip({
  ...
  const cards: {
  ...
  }[] = [
    {
      ...
      body: (
        <>
          ...
          <p className="mt-2 text-xs text-white/45">{copy.emailSub}</p>
        </>
      ),
    },
    {
    ...
    },
    {
      ...
      body: (
        <>
          <p className="mt-3 text-sm leading-relaxed text-white/55">{copy.globalBody}</p>
          <p className="mt-3 text-xs text-white/40">{copy.globalSub}</p>
        </>
      ),
    },
  ];
  ...
  return (
    <section id={id} aria-label="Contact" className={className}>
